...
def parse_args():
    ...
    parser.add_argument(
        "--data-path", type=str, default="DIC_crack_dataset"
    )
...
def main(args):
    ...
    ds = Dataset(
        root=args.data_path,
        fold="train",
        transform=get_train_transform(add_inverse=("bcos" in model_type), size=imagesize),
        segmentation=False,
    ...
    model = Classifier(model=model_type, pretrained=args.pretrained, num_classes=args.num_classes)
    ...
    if args.eval:
        with torch.inference_mode(False):
            model = Classifier.load_from_checkpoint(checkpoint, model=model_type, num_classes=args.num_classes)#.eval()
